...
image = tf.reshape(x_train[randomNum], [3,218,218])
...
sess = tf.Session()
x_placeholder = tf.placeholder("float", shape = [None,3,28,28]) #Placeholder for input images to the discriminator
...
for i in range(iterations):
    ...
    real_image_batch = next_batch(100, dataset)
    try:
        real_image_batch = real_image_batch[0].reshape(28, 28, 1)#[batch_size,3,120,160])
        print('passed')
    except:
        print('failed')
        continue
...
cv2.imshow('image', cv2.imread('sample_image', 0))
